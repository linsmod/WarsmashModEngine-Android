...
public class OpenALAudio implements LwjglAudio, IOpenALAudio {
	...
	public int obtainSource(final boolean isMusic) {
		if (this.noDevice) {
			return 0;
		}
		for (int i = 0, n = this.idleSources.size; i < n; i++) {
			...
			if ((state != AL_PLAYING) && (state != AL_PAUSED)) {
				...
				AL10.alSourcef(sourceId, AL_GAIN, 1);
	...
	public void stopSourcesWithBuffer(final int bufferID) {
		if (this.noDevice) {
			return;
		}
		for (int i = 0, n = this.idleSources.size; i < n; i++) {
	...
	@Override
	public void pauseSourcesWithBuffer(final int bufferID) {
		if (this.noDevice) {
			return;
		}
		for (int i = 0, n = this.idleSources.size; i < n; i++) {
	...
	public void resumeSourcesWithBuffer(final int bufferID) {
		if (this.noDevice) {
			return;
		}
		for (int i = 0, n = this.idleSources.size; i < n; i++) {
...
//				@Override
	 ...
	 */
	public void retain(final OpenALSound sound, final boolean stop) {
	...
	public void playSound(int sourceID, int bufferID, boolean looping, float volume) {
		...
		AL10.alSourcei(sourceID, AL_LOOPING, looping ? AL_TRUE : AL_FALSE);
		AL10.alSourcef(sourceID, AL_GAIN, volume);
