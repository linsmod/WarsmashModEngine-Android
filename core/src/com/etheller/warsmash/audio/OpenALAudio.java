...
public class OpenALAudio implements LwjglAudio, IOpenALAudio {
    ...
    public OpenALSound newSound(final FileHandle file) {
        ...
        try {
            return soundClass.getConstructor(new Class[]{IOpenALAudio.class, FileHandle.class}).newInstance(this,
                    file);
    ...
    public int getAudioSource(final boolean isMusic) {
        if (this.noDevice) {
            return 0;
        }
    ...
    public void freeSource(final int sourceID) {
        if (this.noDevice) {
            return;
        }
        alSourceStop(sourceID);
    ...
    public void CALL_freeBuffer(final int bufferID) {
        if (this.noDevice) {
            return;
        }
        for (int i = 0, n = this.idleSources.size; i < n; i++) {
    ...
    public void CALL_alBufferData(int bufferID, int i, ShortBuffer shortBuffer, int sampleRate) {
        AL10.alBufferData(bufferID, i, shortBuffer, sampleRate);
    }
    ...
    public void playSound(int sourceID, int bufferID, boolean looping, float volume) {
        ...
        com.shc.androidopenal.AL.alSourcePlay(sourceID);
    }
    ...
    public void stopSource(final int bufferID) {
        if (this.noDevice) {
            return;
        }
        for (int i = 0, n = this.idleSources.size; i < n; i++) {
    ...
    public void pauseSource(final int bufferID) {
        if (this.noDevice) {
            return;
        }
        for (int i = 0, n = this.idleSources.size; i < n; i++) {
    ...
    public void resumeSource(final int bufferID) {
        if (this.noDevice) {
            return;
        }
        for (int i = 0, n = this.idleSources.size; i < n; i++) {
     ...
     */
    public void retainSound(final OpenALSound sound, final boolean stop) {
